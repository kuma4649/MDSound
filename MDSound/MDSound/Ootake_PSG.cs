...
namespace MDSound
{
    public class Ootake_PSG : Instrument
    {
        /*-----------------------------------------------------------------------------
            [PSG.h]
        ...
        **---------------------------------------------------------------------------*/
        //Sint32
        //private void PSG_Init(int clock, int sampleRate) { }

        //private void PSG_Deinit(HuC6280 chip) { }

        //private void PSG_Mix(HuC6280 chip, int[][] pDst, int nSample) { }
        //	Sint16*		pDst,				// 出力先バッファ 
        //void* chip,
        //Sint32** pDst,
        ...
        //private byte PSG_Read(HuC6280 chip, uint regNum) { return 0; }

        //private void PSG_Write(HuC6280 chip, uint regNum, byte data) { }
        ...
        //Kitao追加
        //private void PSG_ResetVolumeReg(HuC6280 chip) { }

        //Kitao追加
        //private void PSG_SetMutePsgChannel(HuC6280 chip, int num, bool bMute) { }

        //private void PSG_SetMuteMask(HuC6280 chip, uint MuteMask) { }

        //Kitao追加
        //private bool PSG_GetMutePsgChannel(HuC6280 chip, int num) { return false; }

        //Kitao追加。v2.60
        //private void PSG_SetHoneyInTheSky(HuC6280 chip, bool bHoneyInTheSky) { }
...
******************************************************************************/
        ...
        private int N_CHANNEL = 6;

        //#define SAMPLE_RATE			44100.0 //Kitao更新。現状は速度優先でサンプルレートを44100固定とした。
        private double OVERSAMPLE_RATE = 1.0; //Kitao更新。PSGはオーバーサンプリングすると響きの美しさが損なわれてしまうのでオーバーサンプリングしないようにした。速度的にもアップ。
        private double PSG_DECLINE = (21.8500 * 6.0); //21.8500。Kitao追加。PSG音量の減少値。*6.0は各チャンネル足したぶんを割る意味。大きいほど音は減る。CDDAが100%のときにちょうど良いぐらいの音量に合わせよう。v2.19,v2.37,v2.39,v2.62更新
        private double VOL_TABLE_DECLINE = -1.05809999010; //-1.05809999010で雀探物語２OK。Kitao追加。音量テーブルの減少値。マイナスが大きいほど小さい音が聞こえづらくなる。マイナスが小さすぎると平面的な音になる。v2.19,v2.37,v2.39,v2.40,v2.62,v2.65更新
        ...
        private int NOISE_TABLE_VALUE_front = -18;
        private int NOISE_TABLE_VALUE_rear = -1; //キレと聴きやすさで-18:-1をベストとした。最大値が大きい(+に近い)と重い音に。２つの値が離れていると重い音に。フォーメーションサッカー，大魔界村のエンディングのドラムなどで調整。v1.46,v2.40,v2.62更新
                                                 //  ※VOL_TABLE_DECLINEによってこの値の最適値も変化する。
        private double SAMPLE_FADE_DECLINE = 0.305998999951; //0.30599899951。Kitao追加。サンプリング音の消音時の音の減退量。ソルジャーブレイド,将棋初心者無用の音声で調整。基本的にこの値が小さいほうがノイズが減る(逆のケースもある)。v2.40
        ...
        public class PSG
        {
            ...
            public uint phase;
            public uint deltaPhase;
            public bool bNoiseOn;
            public uint noiseFrq;
        ...
        public class huc6280_state
        {
            ...
            public double RESMPL_RATE;

            public PSG[] Psg = new PSG[8];             // 6, 7 is unused
            public int[] DdaFadeOutL = new int[8]; //Kitao追加
            public int[] DdaFadeOutR = new int[8]; //Kitao追加
            public uint Channel;             // 0 - 5;
            public uint MainVolumeL;         // 0 - 15
            public uint MainVolumeR;         // 0 - 15
            public uint LfoFrq;
            public bool bLfoOn=false; //v1.59から非使用。過去verのステートロードのために残してある。
            public uint LfoCtrl;
            public uint LfoShift=0; //v1.59から非使用。過去verのステートロードのために残してある。
            public int PsgVolumeEffect; // = 0;//Kitao追加
            public double Volume;  // = 0;//Kitao追加
            public double VOL; // = 0.0;//Kitao追加。v1.08
                               //	BOOL		_bPsgMute[8] = {FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE};//Kitao追加。v1.29
            public bool[] bPsgMute = new bool[8];

            public byte[] Port = new byte[16];             // for debug purpose 

            public bool bWaveCrash; //Kitao追加。DDA再生中にWaveデータが書き換えられたらTRUE
            public bool bHoneyInTheSky; //はにいいんざすかいパッチ用。v2.60
        }

        private int[] _VolumeTable = new int[92];
        private int[] _NoiseTable = new int[32768];

        //static BOOL		_bPsgInit = FALSE;
        private bool _bTblInit = false;
        ...
        */
        ...
        private void create_volume_table()
        {
            int i;
            double v;
            ...
            {
                v = 91 - i;
                _VolumeTable[i] = (int)(32768.0 * Math.Pow(10.0, v * VOL_TABLE_DECLINE / 20.0)); //VOL_TABLE_DECLINE。小さくしすぎると音が平面的な傾向に。ソルジャーブレイドで調整。v1.46。
            }
        }


        //ノイズテーブルの作成
        private void create_noise_table()
        {
            int i;
            uint bit0;
            uint bit1;
            ...
            for (i = 0; i < 32768; i++)
            {
                ...
                reg >>= 1;
                reg |= (bit14 << 14);
                _NoiseTable[i] = (bit0 != 0) ? NOISE_TABLE_VALUE_front : NOISE_TABLE_VALUE_rear; //Kitao更新。ノイズのボリュームと音質を調整した。
            }
        }
        ...
        -----------------------------------------------------------------------------*/
        //static inline void
        private void write_reg(huc6280_state info, byte reg, byte data)
        {
            uint i;
            uint frq;//Kitao追加
            PSG PSGChn;
            ...
            switch (reg & 15)
            {
                case 0: // register select
                    info.Channel = (uint)(data & 7);
                    break;

                case 1: // main volume
                    info.MainVolumeL = (uint)((data >> 4) & 0x0F);
                    info.MainVolumeR = (uint)(data & 0x0F);
                ...
                case 2: // frequency low
                    ...
                    PSGChn.frq &= ~(uint)0xFF;
                    PSGChn.frq |= data;
                    //Kitao更新。update_frequencyは、速度アップのためサブルーチンにせず直接実行するようにした。
                    frq = (PSGChn.frq - 1) & 0xFFF;
                    if (frq != 0)
                        PSGChn.deltaPhase = (uint)((double)(65536.0 * 256.0 * 8.0 * info.RESMPL_RATE) / (double)frq + 0.5); //Kitao更新。速度アップのためfrq以外は定数計算にした。精度向上のため、先に値の小さいOVERSAMPLE_RATEのほうで割るようにした。+0.5は四捨五入で精度アップ。プチノイズ軽減のため。
                    else
                        PSGChn.deltaPhase = 0;
                    break;
                ...
                case 3: // frequency high
                    PSGChn = info.Psg[info.Channel];
                    PSGChn.frq &= ~(uint)0xF00;
                    PSGChn.frq |= (uint)((data & 0x0F) << 8);
                    //Kitao更新。update_frequencyは、速度アップのためサブルーチンにせず直接実行するようにした。
                    frq = (PSGChn.frq - 1) & 0xFFF;
                    if (frq != 0)
                        PSGChn.deltaPhase = (uint)((double)(65536.0 * 256.0 * 8.0 * info.RESMPL_RATE) / (double)frq + 0.5); //Kitao更新。速度アップのためfrq以外は定数計算にした。精度向上のため、先に値の小さいOVERSAMPLE_RATEのほうで割るようにした。+0.5は四捨五入で精度アップ。プチノイズ軽減のため。
                ...
                case 4: // ON, DDA, AL
                    ...
                    if ((PSGChn.bDDA) && ((data & 0x40) == 0)) //DDAからWAVEへ切り替わるとき or DDAから消音するとき
                    {
                        ...
                        info.DdaFadeOutL[info.Channel] = (int)((double)(PSGChn.ddaSample * PSGChn.outVolumeL) *
                                                                    ((1 + (1 >> 3) + (1 >> 4) + (1 >> 5) + (1 >> 7) + (1 >> 12) + (1 >> 14) + (1 >> 15)) * SAMPLE_FADE_DECLINE)); //元の音量。v2.65更新
                        info.DdaFadeOutR[info.Channel] = (int)((double)(PSGChn.ddaSample * PSGChn.outVolumeR) *
                ...
                case 5: // LAL, RAL
                    PSGChn = info.Psg[info.Channel];
                    PSGChn.volumeL = (uint)((data >> 4) & 0xF);
                    PSGChn.volumeR = (uint)(data & 0xF);
                    PSGChn.outVolumeL = _VolumeTable[PSGChn.volume + (info.MainVolumeL + PSGChn.volumeL) * 2];
                    PSGChn.outVolumeR = _VolumeTable[PSGChn.volume + (info.MainVolumeR + PSGChn.volumeR) * 2];
                    break;
                ...
                case 7: // noise on, noise frq
                    if (info.Channel >= 4)
                    {
                        ...
                        PSGChn.noiseFrq = (uint)(0x1F - (data & 0x1F));
                        if (PSGChn.noiseFrq == 0)
                            PSGChn.deltaNoisePhase = (uint)((double)(2048.0 * info.RESMPL_RATE) + 0.5); //Kitao更新
                        else
                ...
                case 9: // LFO control  Kitao更新。シンプルに実装してみた。実機で同じ動作かは未確認。はにいいんざすかいの音が似るように実装。v1.59
                    ...
                    info.LfoCtrl = (uint)(data & 7); //ドロップロックほらホラで5が使われる。v1.61更新
                    if ((info.LfoCtrl & 4) != 0)
        ...
        //Kitao追加
        private void set_VOL(huc6280_state info)
        {
            //Sint32	v;
        ...
        /*-----------------------------------------------------------------------------
            [Mix]
                ＰＳＧの出力をミックスします。
        -----------------------------------------------------------------------------*/
        private void PSG_Mix(huc6280_state chip, int[][] pDst, int nSample)
        ...
        //Sint32 nSample)	// 書き出すサンプル数 
        {
            ...
            int sample; //Kitao追加
            int lfo;
            int sampleAllL; //Kitao追加。6chぶんのサンプルを足していくためのバッファ。精度を維持するために必要。6chぶん合計が終わった後に、これをSint16に変換して書き込むようにした。
            int sampleAllR; //Kitao追加。上のＲチャンネル用
            int smp; //Kitao追加。DDA音量,ノイズ音量計算用
            int[] bufL = pDst[0];
            int[] bufR = pDst[1];
            ...
            for (j = 0; j < nSample; j++)
            {
                    ...
                    if ((PSGChn.bOn) && ((i != 1) || (info.LfoCtrl == 0)) && (!info.bPsgMute[i])) //Kitao更新
                    {
                        ...
                        else if (PSGChn.deltaPhase != 0)
                        {
                            ...
                            if ((i == 0) && (info.LfoCtrl > 0))
                            {
                                ...
                                lfo = info.Psg[1].wave[info.Psg[1].phase >> 27] << (int)((info.LfoCtrl - 1) << 1); //v1.60更新
                                info.Psg[0].phase += (uint)((double)(65536.0 * 256.0 * 8.0 * info.RESMPL_RATE) / (double)(info.Psg[0].frq + lfo) + 0.5);
                                info.Psg[1].phase += (uint)((double)(65536.0 * 256.0 * 8.0 * info.RESMPL_RATE) / (double)(info.Psg[1].frq * info.LfoFrq) + 0.5); //v1.60更新
                            }
                            else
                ...
                sampleAllL = (int)((double)sampleAllL * info.VOL);
                ...
                sampleAllR = (int)((double)sampleAllR * info.VOL);
        ...
        //Kitao更新
        private void psg_reset(huc6280_state info)
        {
            int i, j;
            ...
            info.DdaFadeOutL = new int[8]; //Kitao追加
            info.DdaFadeOutR = new int[8]; //Kitao追加
            info.MainVolumeL = 0;
            info.MainVolumeR = 0;
            info.LfoFrq = 0;
            info.LfoCtrl = 0;
        ...
        //private void PSG_SetVolume(huc6280_state info) { }
        /*-----------------------------------------------------------------------------
            [Init]
                ＰＳＧを初期化します。
        -----------------------------------------------------------------------------*/
        //Sint32
        private huc6280_state PSG_Init(int clock, int sampleRate)
        {

            huc6280_state info;

            info = new huc6280_state();
            if (info == null)
                return null;

            info.PSG_FRQ = clock & 0x7FFFFFFF;
            PSG_SetHoneyInTheSky(info, ((clock >> 31) & 0x01) != 0);
        ...
        /*-----------------------------------------------------------------------------
            [Deinit]
                ＰＳＧを破棄します。
        -----------------------------------------------------------------------------*/
        private void PSG_Deinit(huc6280_state chip)
        {
            huc6280_state info = chip;
        ...
        -----------------------------------------------------------------------------*/
        private byte PSG_Read(huc6280_state chip, uint regNum)
        {
            huc6280_state info = chip;

            if (regNum == 0)
                return (byte)info.Channel;
        ...
        /*-----------------------------------------------------------------------------
            [Write]
                ＰＳＧポートの書き込みに対する動作を記述します。
        -----------------------------------------------------------------------------*/
        private void PSG_Write(huc6280_state chip, uint regNum, byte data)
        {
            huc6280_state info = chip;
            ...
            */
            write_reg(chip, (byte)regNum, data);
        }
        ...
        //Kitao追加
        private void PSG_ResetVolumeReg(huc6280_state chip)
        {
            huc6280_state info = chip;
            int i;
        ...
        //Kitao追加
        private void PSG_SetMutePsgChannel(huc6280_state chip, int num, bool bMute)
        {
            huc6280_state info = chip;
        ...
        private void PSG_SetMuteMask(huc6280_state chip, uint MuteMask)
        {
            byte CurChn;

            for (CurChn = 0x00; CurChn < N_CHANNEL; CurChn++)
                PSG_SetMutePsgChannel(chip, CurChn, ((MuteMask >> CurChn) & 0x01) != 0);

            return;
        }

        //Kitao追加
        private bool PSG_GetMutePsgChannel(huc6280_state chip, int num)
        {
            huc6280_state info = chip;
        ...
        //Kitao追加。v2.60
        private void PSG_SetHoneyInTheSky(huc6280_state chip, bool bHoneyInTheSky)
        {
            huc6280_state info = chip;
        ...
        public void HuC6280_SetMute(byte ChipID, int val)
        {
            ...
            PSG_SetMuteMask(chip[ChipID], (uint)val);
