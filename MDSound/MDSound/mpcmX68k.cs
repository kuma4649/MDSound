...
namespace MDSound
{
    public class mpcmX68k : Instrument
    {
        ...
        public override uint Start(byte ChipID, uint samplingRate, uint clock, params object[] option)
        {
            ...
            Initialize(ChipID, (int)clock, samplingRate);
            return samplingRate;
        }
        ...
        public M[] m = new M[MAX_CHIPS] { new M(), new M() };




        // $Id: ssMPCM.h,v 1.4 2016/03/14 16:13:40 saya Exp $
        ...
        public class SETPCM
        {
            ...
            public int adrs_ptr;
            public UInt32 size;
            public UInt32 start;        // ループ開始点
            public UInt32 end;      // ループ終点
            public UInt32 count;        // ループ回数(0:無限)
        }

        public enum VOICE
        {
            MAX = 16         // 効果音用の chは未サポート
        }
        ...
        public class CHWORK
        {
            ...
            public UInt32 size;
            public UInt32 lp_start;
            public UInt32 lp_end;
            public UInt32 lp_count;
            public UInt32 lp_work;
            public UInt32 pitch;
            public Int64 pos;
            public Int64 ppos;
            public int offset;
        ...
        // $Id: ssMPCM.cpp,v 1.9 2016/05/08 01:01:29 saya Exp $
        //
        ...
        public UInt16[] pitchtbl = new UInt16[768] {
        ...
        public int[] NextTable = new int[] {
            -1, -1, -1, -1, 2, 4, 6, 8,
            -1, -1, -1, -1, 2, 4, 6, 8
        };

        public int[] OffsetTable = new int[]{
            0,
            0,  1,  2,  3,  4,  5,  6,  7,
        ...
        {
            m[chipID].work = new CHWORK[(int)VOICE.MAX];
            for(int i=0;i< (int)VOICE.MAX; i++)
            {
                m[chipID].work[i] = new CHWORK();
            }
            m[chipID].DiffTable = new int[(int)TBL.DIFF];
        }

        public void UnmountMpcmX68k(int chipID)
        {
            m[chipID].DiffTable = null;
            m[chipID].work = null;
        }

        public bool Initialize(int chipID, int base_, float sampling_rate)
        {
            m[chipID].rate = sampling_rate;
            m[chipID].base_ = (float)base_ / m[chipID].rate;
        ...
        public void Reset_(int chipID)
        {
            ...
            for (int ch = 0; ch < (int)VOICE.MAX; ch++)
            {
                KeyOff(chipID, ch);
                SetVol(chipID, ch, 64);
        ...
        public void SetVolTable(int chipID, int sel, UInt16[] tbl)
        {
            if (sel == 1)
            {
                // 16
        ...
        private int Decode(int chipID, int ch, byte[] adrs_buf, int adrs_ptr, long pos)
        {
            ...
            byte data;
            long cnt = 0;
            long prev = m[chipID].work[ch].ppos;
            int sample = m[chipID].work[ch].sample;
            int offset = m[chipID].work[ch].offset;
            int diff = 0;
            //int store = 0;
            //int poffset = 0;
            ...
            {
                data = adrs_buf[(uint)(adrs_ptr + ((prev + c) >> 1))];
                if (((prev + c) & 1) != 0)
                {
                ...
                // 偶数番値の場合はループ位置判定をする
                if (((prev + c) & 1) == 0 && (m[chipID].work[ch].lp_start == pos))
                {
                    m[chipID].work[ch].lp_sample = sample;
                    m[chipID].work[ch].lp_offset = offset;
                }
            ...
            m[chipID].work[ch].offset = offset;
            m[chipID].work[ch].sample = sample;
            m[chipID].work[ch].ppos = pos;
        ...
        public void Update_(int chipID, int[][] _buffer, Int32 _count)
        {
            ...
            for (int ch = 0; ch < (int)VOICE.MAX; ch++)
            {
                bool mute = ((Int32)m[chipID].mask & (1 << ch)) != 0;
                buf_ptr = 0;

                byte[] ptr_buf = m[chipID].work[ch].adrs_buf;
                int ptr_ptr = m[chipID].work[ch].adrs_ptr;
                long ofst = m[chipID].work[ch].pos;
                ...
                for (UInt32 bufsize = 0; (bufsize < _count) && (m[chipID].work[ch].enable); bufsize++)
                {
                    ...
                    ofst += pitch;
                    if ((ofst >> 16) > m[chipID].work[ch].lp_end)
                    {
                        m[chipID].work[ch].lp_work++;
                        if ((m[chipID].work[ch].lp_count != 0) && (m[chipID].work[ch].lp_work >= m[chipID].work[ch].lp_count))
                        {
                            KeyOff(chipID, ch);
                        }
                        else
                        {
                            ofst &= 0xffff;
                            ofst += (m[chipID].work[ch].lp_start << 16);
        ...
        public Int16 GetBufferFlag(int _b)
        {
            return 0;
        }
