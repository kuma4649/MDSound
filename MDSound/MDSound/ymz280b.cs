...
namespace MDSound
{
    public class ymz280b : Instrument
    {
        ...
        {
            return (UInt32)device_start_ymz280b(ChipID, 16934400);
        }

        public override uint Start(byte ChipID, uint clock, uint FMClockValue, params object[] option)
        {
            return (UInt32)device_start_ymz280b(ChipID, (Int32)FMClockValue);
        }
        ...
        private int YMZ280B_Write(byte ChipID, byte Offset, byte Data)
        {
 ...
 **********************************************************************************************/
        ...
        //#include "devcb.h"
        public delegate void callback(Int32 state);

        public class ymz280b_interface
        {
        ...
        //private void ymz280b_update(byte ChipID, Int32[][] outputs, Int32 samples) { }
        //private int device_start_ymz280b(byte ChipID, Int32 clock) { return 0; }
        //private void device_stop_ymz280b(byte ChipID) { }
        //private void device_reset_ymz280b(byte ChipID) { }

        //private byte ymz280b_r(byte ChipID, Int32 offset) { return 0; }
        //private void ymz280b_w(byte ChipID, Int32 offset, byte data) { }
        //private void ymz280b_write_rom(byte ChipID, Int32 ROMSize, Int32 DataStart, Int32 DataLength, byte[] ROMData) { }

        //private void ymz280b_set_mute_mask(byte ChipID, UInt32 MuteMask) { }
...
*/
        ...
        private const Int32 MAX_SAMPLE_CHUNK = 0x10000;
        private const Int32 MAKE_WAVS = 0;
        private const Int32 MAKE_WAVS_CH = 0;

        private const Int32 FRAC_BITS = 14;
        private const Int32 FRAC_ONE = (1 << FRAC_BITS);
        private const Int32 FRAC_MASK = (FRAC_ONE - 1);

        private const Int32 INTERNAL_BUFFER_SIZE = (1 << 15);
        //#define INTERNAL_SAMPLE_RATE	(chip.master_clock * 2.0)
        //private const Int32 INTERNAL_SAMPLE_RATE = chip.rate;
        ...
        public class YMZ280BVoice
        {
            public byte playing;          /* 1 if we are actively playing */

            public byte keyon;            /* 1 if the key is on */
            public byte looping;          /* 1 if looping is enabled */
            public byte mode;             /* current playback mode */
            public UInt16 fnum;            /* frequency */
            public byte level;            /* output level */
            public byte pan;              /* panning */

            public UInt32 start;           /* start address, in nibbles */
            public UInt32 stop;            /* stop address, in nibbles */
            public UInt32 loop_start;      /* loop start address, in nibbles */
            public UInt32 loop_end;        /* loop end address, in nibbles */
            public UInt32 position;        /* current position, in nibbles */

            public Int32 signal;           /* current ADPCM signal */
            public Int32 step;             /* current ADPCM step */

            public Int32 loop_signal;      /* signal at loop start */
            public Int32 loop_step;        /* step at loop start */
            public UInt32 loop_count;      /* number of loops so far */

            public Int32 output_left;      /* output volume (left) */
            public Int32 output_right;     /* output volume (right) */
            public Int32 output_step;      /* step value for frequency conversion */
            public Int32 output_pos;       /* current fractional position */
            public Int16 last_sample;      /* last sample output */
            public Int16 curr_sample;      /* current sample target */
            public byte irq_schedule;     /* 1 if the IRQ state is updated by timer */
            public byte Muted;            /* used for muting */
        };

        public class ymz280b_state
        {
            //sound_stream * stream;			/* which stream are we using */
            public byte[] region_base;             /* pointer to the base of the region */
            public UInt32 region_size;
            public byte current_register;         /* currently accessible register */
            public byte status_register;          /* current status register */
            public byte irq_state;                /* current IRQ state */
            public byte irq_mask;                 /* current IRQ mask */
            public byte irq_enable;               /* current IRQ enable */
            public byte keyon_enable;             /* key on enable */
            public byte ext_mem_enable;           /* external memory enable */
            public byte ext_readlatch;            /* external memory prefetched data */
            public UInt32 ext_mem_address_hi;
            public UInt32 ext_mem_address_mid;
            public UInt32 ext_mem_address;         /* where the CPU can read the ROM */

            public double master_clock;            /* master clock frequency */
            public double rate;
            //void (*irq_callback)(const device_config *, int);		/* IRQ callback */
            public callback irq_callback;       /* IRQ callback */
            public YMZ280BVoice[] voice = new ymz280b.YMZ280BVoice[8]{
                new YMZ280BVoice(),new YMZ280BVoice(),new YMZ280BVoice(),new YMZ280BVoice(),
                new YMZ280BVoice(),new YMZ280BVoice(),new YMZ280BVoice(),new YMZ280BVoice()
            };  /* the 8 voices */
            ...
            public Int16[] scratch;
            //const device_config *device;
        };

        //public void write_to_register(ymz280b_state chip, Int32 data) { }


        /* step size index shift table */
        private Int32[] index_scale = new Int32[8] { 0x0e6, 0x0e6, 0x0e6, 0x0e6, 0x133, 0x199, 0x200, 0x266 };

        /* lookup table for the precomputed difference */
        private Int32[] diff_lookup = new Int32[16];
        private byte lookup_init = 0x00;    /* lookup-table is initialized */

        /* timer callback */
        /* TIMER_CALLBACK( update_irq_state_timer_0 );
         TIMER_CALLBACK( update_irq_state_timer_1 );
         TIMER_CALLBACK( update_irq_state_timer_2 );
         TIMER_CALLBACK( update_irq_state_timer_3 );
         TIMER_CALLBACK( update_irq_state_timer_4 );
         TIMER_CALLBACK( update_irq_state_timer_5 );
         TIMER_CALLBACK( update_irq_state_timer_6 );
         TIMER_CALLBACK( update_irq_state_timer_7 );

         const timer_fired_func update_irq_state_cb[] =
        {
        ...
        private ymz280b_state[] YMZ280BData = new ymz280b_state[2] { new ymz280b_state(), new ymz280b_state() };//[MAX_CHIPS];

        public override string Name { get { return "YMZ280B"; } set { } }
        public override string ShortName { get { return "YMZ"; } set { } }

        /*private ymz280b_state *get_safe_token(const device_config *device)
        {
            assert(device != NULL);
        ...
        private void update_irq_state(ymz280b_state chip)
        {
            Int32 irq_bits = chip.status_register & chip.irq_mask;
        ...
        private void update_step(ymz280b_state chip, YMZ280BVoice voice)
        {
            double frequency;
            ...
            voice.output_step = (Int32)(frequency * (double)FRAC_ONE / chip.rate);// INTERNAL_SAMPLE_RATE);
        }


        private void update_volumes(YMZ280BVoice voice)
        {
            if (voice.pan == 8)
            {
        ...
        private byte ymz280b_read_memory(byte[] _base, UInt32 size, UInt32 offset)
        {
            offset &= 0xFFFFFF;
            if (offset < size)
                return _base[offset];
        ...
        private void update_irq_state_timer_common(ymz280b_state param, Int32 voicenum)
        {
            ymz280b_state chip = (ymz280b_state)param;

            YMZ280BVoice voice = chip.voice[voicenum];

            if (voice.irq_schedule == 0) return;

            voice.playing = 0;
            chip.status_register |= (byte)(1 << voicenum);

            update_irq_state(chip);
            voice.irq_schedule = 0;
        }

        /* TIMER_CALLBACK( update_irq_state_timer_0 ) { update_irq_state_timer_common(ptr, 0); }
         TIMER_CALLBACK( update_irq_state_timer_1 ) { update_irq_state_timer_common(ptr, 1); }
         TIMER_CALLBACK( update_irq_state_timer_2 ) { update_irq_state_timer_common(ptr, 2); }
         TIMER_CALLBACK( update_irq_state_timer_3 ) { update_irq_state_timer_common(ptr, 3); }
         TIMER_CALLBACK( update_irq_state_timer_4 ) { update_irq_state_timer_common(ptr, 4); }
         TIMER_CALLBACK( update_irq_state_timer_5 ) { update_irq_state_timer_common(ptr, 5); }
         TIMER_CALLBACK( update_irq_state_timer_6 ) { update_irq_state_timer_common(ptr, 6); }
         TIMER_CALLBACK( update_irq_state_timer_7 ) { update_irq_state_timer_common(ptr, 7); }*/
        ...
        ***********************************************************************************************/
        ...
        private Int32 generate_adpcm(YMZ280BVoice voice, byte[] _base, UInt32 size, Int16[] buffer, Int32 samples)
        {
            Int32 position = (Int32)voice.position;
            ...
            if (voice.looping == 0)
            {
                ...
                while (samples != 0)
                {
                    ...
                    val = ymz280b_read_memory(_base, size, (UInt32)(position / 2)) >> ((~position & 1) << 2);
            ...
            else
            {
                ...
                while (samples != 0)
                {
                    ...
                    if (position >= voice.loop_end)
                    {
                        if (voice.keyon != 0)
                        {
                            position = (Int32)voice.loop_start;
            ...
            voice.position = (UInt32)position;
            voice.signal = signal;
            voice.step = step;
        ...
        ***********************************************************************************************/
        ...
        private Int32 generate_pcm8(YMZ280BVoice voice, byte[] _base, UInt32 size, Int16[] buffer, Int32 samples)
        {
            Int32 position = (Int32)voice.position;
            Int32 val;
            Int32 ptrBuffer = 0;
            ...
            if (voice.looping == 0)
            {
                ...
                while (samples != 0)
                {
                    ...
                    //val = base[position / 2];
                    val = ymz280b_read_memory(_base, size, (UInt32)(position / 2));

                    /* output to the buffer, scaling by the volume */
                    buffer[ptrBuffer++] = (Int16)(val * 256);
                    samples--;
        ...
        ***********************************************************************************************/
        ...
        private Int32 generate_pcm16(YMZ280BVoice voice, byte[] _base, UInt32 size, Int16[] buffer, Int32 samples)
        {
            Int32 position = (Int32)voice.position;
            Int32 val;
            Int32 ptrBuffer = 0;
            ...
            if (voice.looping == 0)
            {
                ...
                while (samples != 0)
                {
                    ...
                    val = (Int16)((ymz280b_read_memory(_base, size, (UInt32)(position / 2 + 0)) << 8) + ymz280b_read_memory(_base, size, (UInt32)(position / 2 + 1)));
                    // Note: Last MAME updates say it's: ((position / 2 + 1) << 8) + (position / 2 + 0);

                    /* output to the buffer, scaling by the volume */
                    buffer[ptrBuffer++] = (Int16)val;
                    samples--;
            ...
            else
            {
                ...
                while (samples != 0)
                {
                    /* fetch the current value */
                    //val = (Int16)((base[position / 2 + 1] << 8) + base[position / 2 + 0]);
                    val = (Int16)((ymz280b_read_memory(_base, size, (UInt32)(position / 2 + 0)) << 8)
                                + ymz280b_read_memory(_base, size, (UInt32)(position / 2 + 1)));
        ...
        ***********************************************************************************************/
        ...
        // STREAM_UPDATE( ymz280b_update )
        public void ymz280b_update(byte ChipID, Int32[][] outputs, Int32 samples)
        {
            //ymz280b_state *chip = (ymz280b_state *)param;
            ymz280b_state chip = YMZ280BData[ChipID];
            Int32[] lacc = outputs[0];
            Int32[] racc = outputs[1];
            Int32 v;
            ...
            for (v = 0; v < 8; v++)
            {
                ...
                final_pos = (UInt32)(voice.output_pos + remaining * voice.output_step);
                ...
                {
                    case 0x81:
                        samples_left = (UInt32)generate_adpcm(voice, chip.region_base, chip.region_size, chip.scratch, (Int32)new_samples);
                        break;
                    case 0x82:
                        samples_left = (UInt32)generate_pcm8(voice, chip.region_base, chip.region_size, chip.scratch, (Int32)new_samples);
                        break;
                    case 0x83:
                        samples_left = (UInt32)generate_pcm16(voice, chip.region_base, chip.region_size, chip.scratch, (Int32)new_samples);
                        break;
                    default:
                        samples_left = 0;
                        for (int i = 0; i < new_samples; i++) chip.scratch[i] = 0;
                        //memset(chip.scratch, 0, new_samples * sizeof(chip.scratch[0]));
                        break;
                }
                ...
                if (samples_left != 0)
                {
                    ...
                    samples_left &= 0xffff;
                    _base = (Int32)(new_samples - samples_left);
                    t = (_base == 0) ? curr : chip.scratch[_base - 1];

                    for (i = 0; i < samples_left; i++)
                    {
                        if (t < 0) t = -((-t * 15) >> 4);
                        else if (t > 0) t = (t * 15) >> 4;
                        chip.scratch[_base + i] = (Int16)t;
                    }
                ...
                prev = curr;
                curr = curr_data[ptrCurr_data++];// *curr_data++;
        ...
        ***********************************************************************************************/
        ...
        // DEVICE_START( ymz280b )
        public Int32 device_start_ymz280b(byte ChipID, Int32 clock)
        {
            //ymz280b_interface defintrf = null;// { 0 };
            //const ymz280b_interface *intf = (device->_config != NULL) ? (const ymz280b_interface *)device->_config : &defintrf;
            //ymz280b_interface intf = null;// &defintrf;
            ...
            chip.scratch = new Int16[MAX_SAMPLE_CHUNK];// malloc(MAX_SAMPLE_CHUNK * sizeof(Int16));
            for (Int32 i = 0; i < MAX_SAMPLE_CHUNK; i++) chip.scratch[i] = 0x00;
            ...
            return (Int32)chip.rate;// INTERNAL_SAMPLE_RATE;
        }
        ...
        public void device_reset_ymz280b(byte ChipID)
        {
            ...
            for (i = 0; i < 8; i++)
            {
                YMZ280BVoice voice = chip.voice[i];
            ...
            if (chip.current_register < 0x80)
            {
                ...
                switch (chip.current_register & 0xe3)
                {
                    case 0x00:      /* pitch low 8 bits */
                        voice.fnum = (UInt16)((voice.fnum & 0x100) | (data & 0xff));

                        update_step(chip, voice);
                        break;

                    case 0x01:      /* pitch upper 1 bit, loop, key on, mode */
                        voice.fnum = (UInt16)((voice.fnum & 0xff) | ((data & 0x01) << 8));
                    ...
                    case 0x20:      /* start address high */
                        voice.start = (UInt32)((voice.start & (UInt32)(0x00ffff << 1)) | (UInt32)(data << 17));
                        break;

                    case 0x21:      /* loop start address high */
                        voice.loop_start = (UInt32)((voice.loop_start & (UInt32)(0x00ffff << 1)) | (UInt32)(data << 17));
                        break;

                    case 0x22:      /* loop end address high */
                        voice.loop_end = (UInt32)((voice.loop_end & (UInt32)(0x00ffff << 1)) | (UInt32)(data << 17));
                        break;

                    case 0x23:      /* stop address high */
                        voice.stop = (UInt32)((voice.stop & (UInt32)(0x00ffff << 1)) | (UInt32)(data << 17));
                        break;

                    case 0x40:      /* start address middle */
                        voice.start = (UInt32)((voice.start & (UInt32)(0xff00ff << 1)) | (UInt32)(data << 9));
                        break;

                    case 0x41:      /* loop start address middle */
                        voice.loop_start = (UInt32)((voice.loop_start & (UInt32)(0xff00ff << 1)) | (UInt32)(data << 9));
                        break;

                    case 0x42:      /* loop end address middle */
                        voice.loop_end = (UInt32)((voice.loop_end & (UInt32)(0xff00ff << 1)) | (UInt32)(data << 9));
                        break;

                    case 0x43:      /* stop address middle */
                        voice.stop = (UInt32)((voice.stop & (UInt32)(0xff00ff << 1)) | (UInt32)(data << 9));
                        break;

                    case 0x60:      /* start address low */
                        voice.start = (UInt32)((voice.start & (UInt32)(0xffff00 << 1)) | (UInt32)(data << 1));
                        break;

                    case 0x61:      /* loop start address low */
                        voice.loop_start = (UInt32)((voice.loop_start & (UInt32)(0xffff00 << 1)) | (UInt32)(data << 1));
                        break;

                    case 0x62:      /* loop end address low */
                        voice.loop_end = (UInt32)((voice.loop_end & (UInt32)(0xffff00 << 1)) | (UInt32)(data << 1));
                        break;

                    case 0x63:      /* stop address low */
                        voice.stop = (UInt32)((voice.stop & (UInt32)(0xffff00 << 1)) | (UInt32)(data << 1));
                        break;
            ...
            else
            {
                switch (chip.current_register)
                {
                    ...
                    case 0x84:      /* ROM readback / RAM write (high) */
                        chip.ext_mem_address_hi = (UInt32)(data << 16);
                        break;

                    case 0x85:      /* ROM readback / RAM write (middle) */
                        chip.ext_mem_address_mid = (UInt32)(data << 8);
                        break;

                    case 0x86:      /* ROM readback / RAM write (low) -> update latch */
                        chip.ext_mem_address = (UInt32)(chip.ext_mem_address_hi | chip.ext_mem_address_mid | (UInt32)data);
                    ...
                    case 0xfe:      /* IRQ mask */
                        chip.irq_mask = (byte)data;

                        update_irq_state(chip);
                        break;

                    case 0xff:      /* IRQ enable, test, etc */
                        chip.ext_mem_enable = (byte)((data & 0x40) >> 6);
                        chip.irq_enable = (byte)((data & 0x10) >> 4);
                        ...
                        else if (chip.keyon_enable == 0 && (data & 0x80) != 0)
                        {
                            for (i = 0; i < 8; i++)
                            {
                                if (chip.voice[i].keyon != 0 && chip.voice[i].looping != 0)
                                    chip.voice[i].playing = 1;
                            }
                        }
                        chip.keyon_enable = (byte)((data & 0x80) >> 7);
                        break;
        ...
        ***********************************************************************************************/
        ...
        private Int32 compute_status(ymz280b_state chip)
        {
            byte result;
        ...
        ***********************************************************************************************/
        ...
        //READ8_DEVICE_HANDLER( ymz280b_r )
        public byte ymz280b_r(byte ChipID, Int32 offset)
        {
            //ymz280b_state *chip = get_safe_token(device);
            ymz280b_state chip = YMZ280BData[ChipID];
            ...
            {
                return (byte)compute_status(chip);
            }
        }


        //WRITE8_DEVICE_HANDLER( ymz280b_w )
        public void ymz280b_w(byte ChipID, Int32 offset, byte data)
        {
            //ymz280b_state *chip = get_safe_token(device);
            ymz280b_state chip = YMZ280BData[ChipID];
        ...
        public void ymz280b_write_rom(byte ChipID, Int32 ROMSize, Int32 DataStart, Int32 DataLength, byte[] ROMData)
        {
            ymz280b_state chip = YMZ280BData[ChipID];

            if (chip.region_size != ROMSize)
            {
                chip.region_base = new byte[ROMSize];// (byte*)realloc(chip.region_base, ROMSize);
                chip.region_size = (UInt32)ROMSize;
                for (Int32 i = 0; i < ROMSize; i++) chip.region_base[i] = 0xff;
            ...
            for (Int32 i = 0; i < DataLength; i++) chip.region_base[i + DataStart] = ROMData[i];
            //memcpy(chip.region_base + DataStart, ROMData, DataLength);

            return;
        }

        public void ymz280b_write_rom(byte ChipID, Int32 ROMSize, Int32 DataStart, Int32 DataLength, byte[] ROMData, Int32 srcStartAddress)
        {
            ymz280b_state chip = YMZ280BData[ChipID];
            ...
            for (Int32 i = 0; i < DataLength; i++) chip.region_base[i + DataStart] = ROMData[i + srcStartAddress];
            //memcpy(chip.region_base + DataStart, ROMData, DataLength);
        ...
        public void ymz280b_set_mute_mask(byte ChipID, UInt32 MuteMask)
        {
            ymz280b_state chip = YMZ280BData[ChipID];
            byte CurChn;
        ...
        {
            return YMZ280B_Write(ChipID, (byte)adr, (byte)data);
        }
