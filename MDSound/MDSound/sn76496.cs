...
namespace MDSound
{
    public class SN76496 : Instrument
    {
        ...
        {
            if (chips[ChipID] == null) return;
            sn76496_shutdown(chips[ChipID]);
        }

        public override void Update(byte ChipID, int[][] outputs, int samples)
        {
            if (chips[ChipID] == null) return;
            SN76496Update(chips[ChipID], outputs, samples);
        }
        ...
        private const int MAX_OUTPUT = 0x8000;
        //private int NOISEMODE() { return (R.Register[6] & 4) != 0 ? 1 : 0; }


        //typedef struct _sn76496_state sn76496_state;
        public class sn76496_state
        {
            //sound_stream * Channel;
            public Int32[] VolTable = new int[16]; /* volume table (for 4-bit to db conversion)*/
            public Int32[] Register = new int[8];  /* registers */
            public Int32 LastRegister; /* last register written */
            public Int32[] Volume = new int[4];    /* db volume of voice 0-2 and noise */
            public UInt32 RNG;         /* noise generator LFSR*/
            public Int32 ClockDivider; /* clock divider */
            public Int32 CurrentClock;
            public Int32 FeedbackMask; /* mask for feedback */
            public Int32 WhitenoiseTap1;   /* mask for white noise tap 1 (higher one, usually bit 14) */
            public Int32 WhitenoiseTap2;   /* mask for white noise tap 2 (lower one, usually bit 13)*/
            public Int32 Negate;       /* output negate flag */
            public Int32 Stereo;       /* whether we're dealing with stereo or not */
            public Int32 StereoMask;   /* the stereo output mask */
            public Int32[] Period = new int[4];    /* Length of 1/2 of waveform */
            public Int32[] Count = new int[4];     /* Position within the waveform */
            public Int32[] Output = new int[4];    /* 1-bit output of each channel, pre-volume */
            public Int32 CyclestoREADY;/* number of cycles until the READY line goes active */
            public Int32 Freq0IsMax;   /* flag for if frequency zero acts as if it is one more than max (0x3ff+1) or if it acts like 0 */
            public UInt32[] MuteMsk = new uint[4];
            public byte NgpFlags;     /* bit 7 - NGP Mode on/off, bit 0 - is 2nd NGP chip */
            public sn76496_state NgpChip2;    /* Pointer to other Chip */
        };


        private static sn76496_state LastChipInit = null;
        private static ushort FNumLimit;
        ...
        private byte sn76496_ready_r(sn76496_state chip, int offset)
        {
            //sn76496_state *R = get_safe_token(device);
            sn76496_state R = (sn76496_state)chip;
            //stream_update(R->Channel);
            return (byte)(R.CyclestoREADY != 0 ? 0 : 1);
        }

        //WRITE8_DEVICE_HANDLER( sn76496_stereo_w )
        private void sn76496_stereo_w(sn76496_state chip, int offset, byte data)
        {
            //sn76496_state *R = get_safe_token(device);
            sn76496_state R = (sn76496_state)chip;
            //stream_update(R->Channel);
            if (R.Stereo != 0) R.StereoMask = data;
#if DEBUG
        ...
        private void sn76496_write_reg(sn76496_state chip, int offset, byte data)
        {
            //sn76496_state *R = get_safe_token(device);
            sn76496_state R = (sn76496_state)chip;
            int n, r, c;
        ...
        void SN76496Update(sn76496_state chip, int[][] outputs, int samples)
        {
            int i;
            ...
            NGPMode = (byte)((R.NgpFlags >> 7) & 0x01);
            R2 = R.NgpChip2;
            ...
            while (samples > 0)
            {
                ...
                if (R.Count[3] <= 0)
                {
                    ...
                    if ((
                        ((R.RNG & R.WhitenoiseTap1) != 0 ? 1 : 0)
                        ^
                        (((R.RNG & R.WhitenoiseTap2) != 0 ? 1 : 0) * ((R.Register[6] & 4) != 0 ? 1 : 0))
                        ) != 0)
                    {
                        R.RNG >>= 1;
                        R.RNG |= (uint)R.FeedbackMask;
                ...
                if (R.NgpFlags == 0)
                {
                    for (i = 0; i < 4; i++)
                    {
                        ...
                        vol[i] &= (int)R.MuteMsk[i];
                        // --- Preparation End ---

                        if (R.Stereo != 0)
                        {
                            ggst[0] = (R.StereoMask & (0x10 << i)) != 0 ? 0x01 : 0x00;
                            ggst[1] = (R.StereoMask & (0x01 << i)) != 0 ? 0x01 : 0x00;
                        }
                        if (R.Period[i] > 1 || i == 3)
                        {
                            _out += vol[i] * R.Volume[i] * ggst[0];
                            _out2 += vol[i] * R.Volume[i] * ggst[1];
                        }
                        else if (R.MuteMsk[i] != 0)
                        {
                            // Make Bipolar Output with PCM possible
                            //out += (2 * R.Volume[i] - R.VolTable[5]) * ggst[0];
                            //out2 += (2 * R.Volume[i] - R.VolTable[5]) * ggst[1];
                            _out += R.Volume[i] * ggst[0];
                            _out2 += R.Volume[i] * ggst[1];
                        }
                    }
                }
                else
                {
                    if ((R.NgpFlags & 0x01) == 0)
                    {
                        // Tone Channel 1-3
                        if (R.Stereo != 0)
                        {
                        ...
                        for (i = 0; i < 3; i++)
                        {
                            ...
                            if (R.Period[i] <= FNumLimit && R.Period[i] != 0)
                                vol[i] = 0;
                            vol[i] &= (int)R.MuteMsk[i];
                            // --- Preparation End ---
                    ...
                    else
                    {
                        ...
                        if (R.Stereo != 0)
                        {
                            ggst[0] = (R.StereoMask & 0x80) != 0 ? 0x01 : 0x00;
                            ggst[1] = (R.StereoMask & 0x08) != 0 ? 0x01 : 0x00;
                        }
                        else
                        {
                            ggst[0] = 0x01;
                        ...
                        _out2 += vol[3] * R.Volume[3] * ggst[1];
                    }
                }
                // --- CUSTOM CODE END --

                if (R.Negate != 0) { _out = -_out; _out2 = -_out2; }
        ...
        private void SN76496_set_gain(sn76496_state R, int gain)
        {
            int i;
            double _out;
            ...
            for (i = 0; i < 15; i++)
            {
                ...
                if (_out > MAX_OUTPUT / 4) R.VolTable[i] = MAX_OUTPUT / 4;
                //else R->VolTable[i] = out;
                else R.VolTable[i] = (int)(_out + 0.5); // I like rounding
        ...
        private int SN76496_init(int clock, sn76496_state R, int stereo)
        {
            ...
            for (i = 0; i < 4; i++)
            {
                ...
                R.MuteMsk[i] = unchecked((uint)~0x00);
            }
        ...
        private int generic_start(sn76496_state chip, int clock, int feedbackmask, int noisetap1, int noisetap2, int negate, int stereo, int clockdivider, int freq0)
        {
            int sample_rate;
        ...
        private ulong sn76496_start(out sn76496_state chip, int clock, int shiftregwidth, int noisetaps,
                                        int negate, int stereo, int clockdivider, int freq0)
        {
            ...
            chip = null;
            sn_chip = (sn76496_state)new sn76496_state();
            if (sn_chip == null)
                return 0;
        ...
        private void sn76496_reset(sn76496_state chip)
        {
            sn76496_state R;
            byte i;
        ...
        private void sn76496_freq_limiter(int clock, int clockdiv, int sample_rate)
        {
            FNumLimit = (ushort)((clock / (clockdiv != 0 ? 2.0 : 16.0)) / sample_rate);

            return;
        }

        private void sn76496_set_mutemask(sn76496_state chip, uint MuteMask)
        {
            sn76496_state R = (sn76496_state)chip;
            byte CurChn;

            for (CurChn = 0; CurChn < 4; CurChn++)
                R.MuteMsk[CurChn] = (MuteMask & (1 << CurChn)) != 0 ? 0 : unchecked((uint)~0);
